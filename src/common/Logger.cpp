...
FileLogger logger::record("log/yzmond.log");
FileLogger logger::record_sql("log/sql_yzmond.log");
    ...
    : BaseLogger()
{
    ...
    cout << "日志文件名：" << valid_filename << endl;
    _file.open(valid_filename,
               fstream::out);
    assert(!_file.fail());
}

FileLogger::FileLogger(string filename, int filesize) noexcept
    : BaseLogger()
{
    size = filesize;
    string valid_filename(filename.size(), '\0');
